...
function Decode(fPort, bytes, variables) {
  switch (fPort) {
    case 10: // sensor data
      ...
      return {"event":evt, "aqi":aqi, "co2":co2, "temperature":temp, "humidity":hum, "gas1":gas1, "gas2":gas2, "battery":batt};
    case 11: // sensor data
      ...
      return {"tvoc":tvoc, "pm1.0":pm1_0, "pm2.5":pm2_5, "pm10":pm10 }
    case 8: // version
    ...
    case 12: // device settings
      ...
      return { "dataUploadInterval":ult, "statusLED":led, "sensorType":typ, "sensorStatus":sts, "gas1Type":g1t, "gas2Type":g2t }
    case 13: // threshold settings
      switch (bytes[0]) {
        case 0:
          ...
          var hhth = bytes[5];
          var lhth = bytes[5];          
          return { "highTemperatureThreshold":htth, "lowTemperatureThreshold":ltth, "highHumidityThreshold":hhth, "lowHumidityThreshold":lhth };
        case 1:
          var co2th  = bytes[1]<<8|bytes[2];
          var tvocth = bytes[3]<<8|bytes[4];
          var g1th   = (bytes[5]<<8|bytes[6])/1000;
          var g2th   = (bytes[7]<<8|bytes[8])/1000;
          return { "co2Threshold":co2th, "tvocThreshold":tvocth, "gas1Threshold":g1th, "gas2Threshold":g2th };
        case 2:
          var pm1p0th = bytes[1]<<16|bytes[2]<<8|bytes[3];
          var pm2p5th = bytes[4]<<16|bytes[5]<<8|bytes[6];
          var pm10th  = bytes[7]<<16|bytes[8]<<8|bytes[9];
          return { 'pm1.0 Threshold':pm1p0th, 'pm2.5 Threshold':pm2p5th, 'pm10 Threshold':pm10th };
        case 3:
          var co2th  = bytes[1]<<8|bytes[2];
          var tvocth = bytes[3]<<8|bytes[4];
